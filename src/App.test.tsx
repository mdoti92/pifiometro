...
describe('App', () => {
  ...
  it('redirige a login al visitar /groups/:groupId/fixture sin sesion activa', () => {
    mockedUseAuth.mockReturnValue({ session: null, user: null, loading: false })

    renderAppAt('/groups/group-1/fixture')
  ...
  it('redirige a login al visitar /groups/:groupId/stages/:stageId/standings sin sesion activa', () => {
    mockedUseAuth.mockReturnValue({ session: null, user: null, loading: false })

    renderAppAt('/groups/group-1/stages/stage-1/standings')

    expect(screen.getByRole('heading', { name: 'Iniciar sesión' })).toBeInTheDocument()
  })

  it('redirige a login al visitar /groups/:groupId/tournaments/:tournamentId/history sin sesion activa', () => {
    mockedUseAuth.mockReturnValue({ session: null, user: null, loading: false })

    renderAppAt('/groups/group-1/tournaments/tournament-1/history')

    expect(screen.getByRole('heading', { name: 'Iniciar sesión' })).toBeInTheDocument()
  })

  it('redirige a login al visitar /groups/:groupId/tournaments/:tournamentId/matchday-summary sin sesion activa', () => {
    mockedUseAuth.mockReturnValue({ session: null, user: null, loading: false })

    renderAppAt('/groups/group-1/tournaments/tournament-1/matchday-summary')
